...
const isSameOrigin = (window.location.protocol === "http:" || window.location.protocol === "https:") && 
                     (window.location.port === "5000" || window.location.port === "");
const API_BASE = isSameOrigin ? "" : "http://127.0.0.1:5000";
...
function updateConnectionStatus(online = true) {
  if (!clusterStatusBadge || !statusText) return;
  if (online) {
...
async function loadUsers() {
  try {
    ...
    users.forEach(u => {
      const opt = document.createElement("option");
      opt.value = u.user_id;
      opt.textContent = `User #${u.user_id} - ${u.name || "Guest"}`;
    ...
    console.warn("User list fetch delayed until backend responds:", err);
  }
}
...
async function loadEventSeats(eventId) {
  ...
  try {
    ...
    movieTitle.textContent = data.movie_title || `Event #${eventId}`;
    eventHall.textContent = `🏛️ ${data.hall_name || "Main Hall"}`;
    ...
    const totalSeats = currentEventSeats.length || data.total_seats || 150;
    const bookedCount = data.booked_seats_count !== undefined 
      ? data.booked_seats_count 
      : currentEventSeats.filter(s => (s.status || "").toLowerCase() === "booked").length;
...
function renderSeats(seats) {
  ...
  seats.forEach(s => {
    ...
    const bookedUserId = (s.user_id !== undefined && s.user_id !== null) ? s.user_id : s.booked_by_user_id;
    btn.dataset.bookedBy = bookedUserId || "";

    const isBooked = (s.status || "").toLowerCase() === "booked";
    const isMyBooking = isBooked && String(bookedUserId) === String(currentUserId);
    ...
    if (isCurrentlySelected) {
      ...
      btn.disabled = true;
      btn.title = `Booked by User #${bookedUserId}`;
    } else {
...
function handleSeatClick(seatData, btnElement) {
  const isBooked = (seatData.status || "").toLowerCase() === "booked";
  const bookedUserId = (seatData.user_id !== undefined && seatData.user_id !== null) ? seatData.user_id : seatData.booked_by_user_id;
  const isMyBooking = isBooked && String(bookedUserId) === String(currentUserId);
...
if (bookBtn) bookBtn.addEventListener("click", bookSeat);
